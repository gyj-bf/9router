/**
 * CommandCode API → OpenAI response translator
 *
 * CommandCode API upstream emits NDJSON-style AI SDK v5 stream events:
...
export function commandCodeApiToOpenAIResponse(chunk, state) {
  ...
  switch (event.type) {
    ...
    case "finish": {
      // Zero-output guard: if no content was emitted, emit an error
      if (!state.finishReason && state.chunkIndex === 0) {
        out.push(makeChunk(state, { content: "[CommandCode API: empty response received]" }));
      }
